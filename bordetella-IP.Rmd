...
descriptors <- read_excel(paste0(data_folder, "/IP_samples_quantity.xlsx"), range = "M2:R26", col_types = "text") %>%
  mutate(No = as.integer(No), Type = factor(Type, levels = c("No_Serum", "Not_Infected", "Infected_Early", "Infected_Late")))
...
data_main <- data_long_filtered %>% 
  ...
  filter(Type != "NoSerum", `C: Bordetella` == "+") %>%
...
plot_peptides <- function(y_aes, peptides_to_show, scales = "fixed") {
  ...
  data_main %>%
    filter(`T: Protein IDs` %in% peptides_to_show, Type != "No_Serum") %>%
    ggplot(aes(x = Type, y = {{ y_aes }}, color = serum, shape = Bp)) + 
    geom_smooth(aes(group = `T: Protein IDs`), method = "lm", color = "black") +
    geom_jitter(width = 0.2, height = 0) + facet_wrap(~`T: Protein IDs`, scales = scales) + 
