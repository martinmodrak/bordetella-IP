...
```{r}
data_pertussin <- data_main %>% filter(ProteinIDs %in% pertussin_peptides, Type != "No_Serum") 
...
fit_pertussin <- brm(bf(LFQ + 0.5 ~ ProteinIDs*(Infected + Infected_Late), sigma ~ ProteinIDs:Type), family = "lognormal", data = data_pertussin)
```
```{r}
fit_pertussin
