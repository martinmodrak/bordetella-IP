...
```{r}
set_Bp46 <- filter_unrepresented(set[,pData(set)$Bp == "46"])
set_Bp6260 <- filter_unrepresented(set[,pData(set)$Bp == "6260"])
protset_Bp46 <- combineFeatures(set,fun="robust", groupBy = fData(set)$protein, verbose = TRUE)
protset_Bp46 <- combineFeatures(set_Bp46,fun="robust", groupBy = fData(set_Bp46)$protein, verbose = TRUE)
protset_Bp6260 <- suppressWarnings(combineFeatures(set_Bp6260,fun="robust", groupBy = fData(set_Bp6260)$protein))



msqrobsum_result_type_Bp46 <- msqrobsum(data = protset_Bp46, expression ~  (1 | Type) 
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Type')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
msqrobsum_result_infected_Bp46 <- msqrobsum(data = protset_Bp46, expression ~  (1 | Infected) + (1 |Infected_Late)                              
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Infected')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
msqrobsum_result_infected_late_Bp46 <- msqrobsum(data = protset_Bp46, expression ~  (1 | Infected) + (1 |Infected_Late) 
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Infected_Late')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
msqrobsum_result_type_Bp6260 <- msqrobsum(data = protset_Bp6260, expression ~  (1 | Type) 
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Type')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
msqrobsum_result_infected_Bp6260 <- msqrobsum(data = protset_Bp6260, expression ~  (1 | Infected) + (1 |Infected_Late)                              
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Infected')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
msqrobsum_result_infected_late_Bp6260 <- msqrobsum(data = protset_Bp6260, expression ~  (1 | Infected) + (1 |Infected_Late) 
                              ,  mode = 'msqrobsum'
                              , contrasts = c('Infected_Late')
                              ## group by folowing variables,
                              ## they will also be retained in output
...
setdiff(elisa_active_peptides, interesting_proteins_not_adjusted)
```

```{r}
write.csv(rbind(interesting_contrasts, interesting_contrasts_not_adjusted), paste0(data_folder,"/interesting_contrasts.csv"))
```
